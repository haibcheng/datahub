...
CATALINA_OPTS="$CATALINA_OPTS -Dorg.apache.catalina.connector.RECYCLE_FACADES=true -Dorg.apache.catalina.connector.CoyoteAdapter.ALLOW_BACKSLASH=false -Dorg.apache.tomcat.util.buf.UDecoder.ALLOW_ENCODED_SLASH=false -Dorg.apache.coyote.USE_CUSTOM_STATUS_MSG_IN_HEADER=false -Dorg.apache.catalina.STRICT_SERVLET_COMPLIANCE=true"
if $os400; then
...
if [ "$1" = "debug" ] ; then
  if $os400; then
    ...
    if [ "$1" = "-security" ] ; then
      ...
      shift
      eval exec "\"$_RUNJDB\"" "\"$CATALINA_LOGGING_CONFIG\"" $LOGGING_MANAGER "$JAVA_OPTS" \
        "$JMX_OPTS" "$OTEL_AGENT" "$PROMETHEUS_AGENT" \
        "$CATALINA_OPTS" \
  ...
  if [ "$1" = "-security" ] ; then
    ...
    shift
    eval exec "\"$_RUNJAVA\"" "\"$CATALINA_LOGGING_CONFIG\"" $LOGGING_MANAGER "$JAVA_OPTS" \
      "$JMX_OPTS" "$OTEL_AGENT" "$PROMETHEUS_AGENT" \
      "$CATALINA_OPTS" \
      -D$ENDORSED_PROP="\"$JAVA_ENDORSED_DIRS\"" \
      -classpath "\"$CLASSPATH\"" \
  ...
  if [ "$1" = "-security" ] ; then
    ...
    shift
    eval $_NOHUP "\"$_RUNJAVA\"" "\"$CATALINA_LOGGING_CONFIG\"" $LOGGING_MANAGER "$JAVA_OPTS" \
      "$JMX_OPTS" "$OTEL_AGENT" "$PROMETHEUS_AGENT" \
      "$CATALINA_OPTS" \
      -D$ENDORSED_PROP="\"$JAVA_ENDORSED_DIRS\"" \
      -classpath "\"$CLASSPATH\"" \
